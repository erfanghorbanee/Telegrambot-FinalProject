...
def checking_messages(bot, message, types):
    ...
    if get_making_order_by_id(message.chat.id):
        ...
        if message.text == inscriptions.catalog:
            ...
            bot.send_message(
                message.chat.id, inscriptions.unrecognized_message, parse_mode="html"
            )
...
def checking_order_creating_steps(bot, message, types, steps, params):
    if not steps["number_of_departament"]:
        ...
        bot.send_message(
            message.chat.id, inscriptions.payment_system, parse_mode="html"
        )
...
def create_pre_order_markup(types):
    ...
    item1 = types.InlineKeyboardButton(
        inscriptions.order_true_btn, callback_data="order_true"
    )
    item2 = types.InlineKeyboardButton(
        inscriptions.order_false_btn, callback_data="order_false"
    )
...
def catalog_first_prod(bot, message, types):
    try:
        with open(catalog.products[catalog.current_prod][4], "rb") as photo:
            markup = catalog_markup_create(
                message.chat.id,
                catalog.products,
                catalog.current_prod,
                catalog.prod_amount,
                types,
            )
            bot.send_photo(
                ...
                photo,
                "📚 <b>{0[1]}</b>\n{0[2]}".format(
                    catalog.products[catalog.current_prod]
                ),
                reply_markup=markup,
                parse_mode="html",
            )
...
def catalog_markup_create(call, products, current_prod_catalog, last_prod, types):
    ...
    item2 = types.InlineKeyboardButton(
        f"{current_prod_catalog + 1} / {last_prod + 1}", callback_data="nothing"
    )
...
def how_many_in_cart(chat_id, products, current_prod_catalog):
    ...
    try:
        cur_prod_amount = products_in_cart[str(products[current_prod_catalog][0])][7][
            "amount"
        ]
...
def cart_markup_create(types):
    markup = types.InlineKeyboardMarkup(row_width=1)
    item1 = types.InlineKeyboardButton(
        inscriptions.clear_basket, callback_data="clear_basket"
    )
    item2 = types.InlineKeyboardButton(
        inscriptions.make_order, callback_data="make_order"
    )
    markup.add(item1, item2)
    return markup
...
def cart_update(bot, call):
    ...
    if cart.items == {}:
        ...
        bot.send_message(
            call.message.chat.id, inscriptions.cart_is_empty, parse_mode="html"
        )
...
def orders_function(bot, types, message):
    ...
    if not order.is_orders():
        ...
        bot.send_message(
            message.chat.id, inscriptions.some_orders_here, reply_markup=markup
        )
...
def callback_data_pre_order(bot, call):
    if call.data == "order_true":
        ...
        bot.send_message(call.message.chat.id, inscriptions.order_true)
        set_making_order_status_to_user(call.message.chat.id, 0)
        callback_clear_basket(call)
...
def send_to_operators(bot, message):
    ...
    items_text = items_arr[0]
    result_sum = items_arr[1]
    order = get_orders_by_id(message.chat.id)[
        len(get_orders_by_id(message.chat.id)) - 1
    ]
    params_text = ""
    for k in params:
        params_text += "<b>{0}: </b>{1}\n".format(
